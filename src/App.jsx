...
// ─── DATA PARSERS ────────────────────────────────────────────────────────────
// Replace generators with parsers for file/manual input
const parseSalesData = (data) => {
  // Assume CSV-like: h,rev,orders,conv or manual array
  if (typeof data === 'string') {
    // Simple CSV parse simulation
    return data.split('\n').slice(1).map(line => {
      const [h, rev, orders, conv] = line.split(',');
      return { h, rev: parseInt(rev), orders: parseInt(orders), conv: parseFloat(conv) };
    }).slice(0, 24);
  }
  return data || []; // Manual array input
};
const parseInventoryData = (data) => {
  if (typeof data === 'string') {
    return data.split('\n').slice(1).map(line => {
      const [cat, stock, thr, turn] = line.split(',');
      return { cat, stock: parseInt(stock), thr: parseInt(thr), turn: parseFloat(turn) };
    });
  }
  return data || [];
};
const parseCashData = (data) => {
  if (typeof data === 'string') {
    return data.split('\n').slice(1).map((line, i) => {
      const [d, inflow, outflow, bal] = line.split(',');
      return { d: d || `D${i+1}`, inflow: parseInt(inflow), outflow: parseInt(outflow), bal: parseInt(bal) };
    });
  }
  return data || [];
};
const parseSupportData = (data) => {
  const days = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"];
  if (typeof data === 'string') {
    return data.split('\n').slice(1).map((line, i) => {
      const [day, open, resolved, escalated, csat] = line.split(',');
      return { day: day || days[i], open: parseInt(open), resolved: parseInt(resolved), escalated: parseInt(escalated), csat: parseFloat(csat) };
    });
  }
  return data || [];
};
const parseComplaintsData = (data) => {
  if (typeof data === 'string') {
    // Simulate parsing complaints CSV
    return data.split('\n').slice(1).map((line, i) => {
...
const genSales = () =>
  Array.from({ length: 24 }, (_, i) => ({
    h: `${i}:00`,
    rev: Math.floor(Math.random() * (44000 - 8000) + 8000),
    orders: Math.floor(Math.random() * (170 - 18) + 18),
    conv: parseFloat((Math.random() * (9.2 - 1.8) + 1.8).toFixed(1)),
  }));
const genInventory = () => [
  { cat: "Electronics", stock: Math.floor(Math.random() * (400 - 90) + 90), thr: 150, turn: parseFloat((Math.random() * (4.8 - 1.2) + 1.2).toFixed(1)) },
  { cat: "Apparel", stock: Math.floor(Math.random() * (600 - 60) + 60), thr: 100, turn: parseFloat((Math.random() * (3.2 - 0.8) + 0.8).toFixed(1)) },
  { cat: "Home & Garden", stock: Math.floor(Math.random() * (800 - 180) + 180), thr: 200, turn: parseFloat((Math.random() * (2.1 - 0.5) + 0.5).toFixed(1)) },
  { cat: "Sports", stock: Math.floor(Math.random() * (300 - 40) + 40), thr: 80, turn: parseFloat((Math.random() * (5.0 - 1.5) + 1.5).toFixed(1)) },
  { cat: "Food & Bev", stock: Math.floor(Math.random() * (1200 - 260) + 260), thr: 400, turn: parseFloat((Math.random() * (12.0 - 4.0) + 4.0).toFixed(1)) },
];
const genSupport = () => {
  const days = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"];
  return days.map(d => ({
    day: d,
    open: Math.floor(Math.random() * (60 - 5) + 5),
    resolved: Math.floor(Math.random() * (80 - 10) + 10),
    escalated: Math.floor(Math.random() * (15 - 0) + 0),
    csat: parseFloat((Math.random() * (4.9 - 3.1) + 3.1).toFixed(1)),
  }));
};
const genCash = () => {
  let bal = Math.floor(Math.random() * (160000 - 80000) + 80000);
  return Array.from({ length: 30 }, (_, i) => {
    const inflow = Math.floor(Math.random() * (26000 - 5000) + 5000);
    const outflow = Math.floor(Math.random() * (23000 - 4000) + 4000);
...
const genComplaints = () =>
  Array.from({ length: 14 }, (_, i) => {
    ...
    const hoursAgo = Math.floor(Math.random() * (72 - 1) + 1);
    return {
      id: `CMP-${1000 + i}`,
...
// Business Stress Score: Weighted formula (inverted health for stress, higher = more stress)
const calcBusinessStressScore = (sales, inv, sup, cash, complaints) => {
  const avgRev = sales.slice(-6).reduce((s, d) => s + d.rev, 0) / 6;
  const revStress = Math.max(0, 100 - Math.min(100, (avgRev / 44000) * 100)); // High stress if low rev
  const lowStock = inv.filter(i => i.stock < i.thr).length;
  const invStress = (lowStock / inv.length) * 100;
  const lastSup = sup[sup.length - 1] || {};
  const supStress = ((lastSup.escalated || 0) / Math.max(1, lastSup.open) * 100) * 3;
  const lastCash = cash[cash.length - 1] || {};
  const cashStress = Math.max(0, 100 - Math.min(100, (lastCash.bal / 200000) * 100));
  const openComplaints = complaints.filter(c => c.status === "open" || c.status === "escalated").length;
  const complaintStress = (openComplaints / complaints.length) * 150;
  // Weighted: Sales 30%, Inventory 20%, Cash 25%, Support 15%, Complaints 10% (business-justified: revenue/cash core)
  const overallStress = Math.round(
    revStress * 0.30 + cashStress * 0.25 + invStress * 0.20 +
    supStress * 0.15 + complaintStress * 0.10
  );
  return {
    overall: Math.min(100, overallStress),
    breakdown: { 
      sales: Math.round(revStress), 
      inventory: Math.round(invStress), 
      cashflow: Math.round(cashStress), 
      support: Math.round(supStress), 
      complaints: Math.round(complaintStress) 
    },
    label: overallStress > 75 ? "CRITICAL STRESS" : overallStress > 55 ? "HIGH STRESS" : overallStress > 35 ? "MODERATE STRESS" : "LOW STRESS",
    color: overallStress > 75 ? C.red : overallStress > 55 ? C.amber : overallStress > 35 ? C.cyan : C.lime,
  };
};
// Enhanced Alerts: Three-tier - crisis, opportunity, anomaly (e.g., unusual patterns)
const genAlerts = (sales, inv, sup, cash, complaints) => {
  const out = [];
  const lowStock = inv.filter(i => i.stock < i.thr);
  if (lowStock.length) out.push({ type: "crisis", icon: "⚠", title: "Critical Stock Alert", msg: `${lowStock.map(i => i.cat).join(", ")} below reorder threshold`, t: "just now" });
  const lc = cash[cash.length - 1] || {};
  if (lc.bal < 55000) out.push({ type: "crisis", icon: "🔴", title: "Cash Flow Crisis", msg: `Balance at $${lc.bal.toLocaleString()} — runway under 30 days`, t: "2m ago" });
  const ls = sup[sup.length - 1] || {};
  if (ls.escalated > 8) out.push({ type: "crisis", icon: "🚨", title: "Support Overload", msg: `${ls.escalated} escalated tickets need immediate action`, t: "4m ago" });
  const spike = sales.find(s => s.conv > 7.5);
  if (spike) out.push({ type: "opportunity", icon: "🚀", title: "Conversion Spike", msg: `${spike.h} — ${spike.conv}% conversion. Push ad spend now.`, t: "9m ago" });
  const hot = inv.find(i => i.turn > 4);
  if (hot) out.push({ type: "opportunity", icon: "💰", title: "Hot Category", msg: `${hot.cat} turnover ${hot.turn}x — run upsell campaign`, t: "14m ago" });
  const critComplaints = complaints.filter(c => c.priority === "critical" && c.status !== "resolved").length;
  if (critComplaints > 0) out.push({ type: "crisis", icon: "😡", title: "Critical Complaints", msg: `${critComplaints} unresolved critical customer complaints`, t: "1m ago" });
  const angryUnresolved = complaints.filter(c => c.sentiment === "angry" && c.status === "open").length;
  if (angryUnresolved > 2) out.push({ type: "crisis", icon: "🔥", title: "Angry Customers Waiting", msg: `${angryUnresolved} angry customers with no response — act now`, t: "3m ago" });
  // Anomaly: e.g., unusual sales dip or inventory anomaly
  const salesAvg = sales.reduce((s, d) => s + d.rev, 0) / sales.length;
  const salesAnomaly = sales.find(s => Math.abs(s.rev - salesAvg) > salesAvg * 0.5);
  if (salesAnomaly) out.push({ type: "anomaly", icon: "❓", title: "Sales Anomaly Detected", msg: `Unusual revenue at ${salesAnomaly.h}: $${salesAnomaly.rev} (avg $${Math.round(salesAvg)})`, t: "5m ago" });
  return out;
};
// ─── COMPONENTS ───────────────────────────────────────────────────────────────
function DataInputModal({ isOpen, onClose, onDataLoad, vertical }) {
  const [file, setFile] = useState(null);
  const [manualData, setManualData] = useState("");
  const [type, setType] = useState("file"); // file or manual

  const handleLoad = () => {
    if (type === "file" && file) {
      const reader = new FileReader();
      reader.onload = (e) => {
        const parsed = parseVerticalData(vertical, e.target.result);
        onDataLoad(vertical, parsed);
        onClose();
      };
      reader.readAsText(file);
    } else if (type === "manual") {
      const parsed = parseVerticalData(vertical, manualData);
      onDataLoad(vertical, parsed);
      onClose();
    }
  };

  const parseVerticalData = (v, content) => {
    switch (v) {
      case "sales": return parseSalesData(content);
      case "inventory": return parseInventoryData(content);
      case "cash": return parseCashData(content);
      case "support": return parseSupportData(content);
      case "complaints": return parseComplaintsData(content);
      default: return [];
    }
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24, width: 500, maxHeight: "80vh", overflowY: "auto" }}>
        <h3 style={{ fontFamily: C.display, fontSize: 20, marginBottom: 16, color: C.text }}>Load {vertical.toUpperCase()} Data</h3>
        <div style={{ marginBottom: 16 }}>
          <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 8 }}>Input Type</label>
          <select value={type} onChange={(e) => setType(e.target.value)} style={{ width: "100%", padding: "8px", background: C.surface, border: `1px solid ${C.border}`, color: C.text, borderRadius: 4 }}>
            <option value="file">Upload File (CSV)</option>
            <option value="manual">Manual Entry</option>
          </select>
        </div>
        {type === "file" ? (
          <input type="file" accept=".csv,.txt" onChange={(e) => setFile(e.target.files[0])} style={{ width: "100%", padding: "8px", background: C.surface, border: `1px solid ${C.border}`, color: C.text, borderRadius: 4, marginBottom: 16 }} />
        ) : (
          <textarea
            value={manualData}
            onChange={(e) => setManualData(e.target.value)}
            placeholder={`Paste CSV data for ${vertical} (e.g., header: col1,col2,...)`}
            style={{ width: "100%", height: 200, background: C.surface, border: `1px solid ${C.border}`, color: C.text, borderRadius: 4, padding: 8, marginBottom: 16, fontFamily: C.mono }}
          />
        )}
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{ padding: "8px 16px", background: "transparent", border: `1px solid ${C.border}`, color: C.muted, borderRadius: 4, cursor: "pointer" }}>Cancel</button>
          <button onClick={handleLoad} style={{ padding: "8px 16px", background: C.lime, border: "none", color: C.bg, borderRadius: 4, cursor: "pointer", fontWeight: 700 }}>Load Data</button>
        </div>
      </div>
    </div>
  );
...
function LivePreview() {
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      setScore(s => Math.max(42, Math.min(98, s + Math.floor(Math.random() * (4 - (-3)) + (-3)))));
      setRev(r => r + Math.floor(Math.random() * (600 - (-200)) + (-200)));
      setTickets(t => Math.max(8, Math.min(45, t + Math.floor(Math.random() * (2 - (-1)) + (-1)))));
      setCash(c => c + Math.floor(Math.random() * (1200 - (-500)) + (-500)));
    }, 2200);
    ...
    <div style={{ width:320,background:"#0b0f08",border:`1px solid ${C.lime}30`,borderRadius:10,overflow:"hidden",boxShadow:"0 40px 100px rgba(0,0,0,0.7)",animation:"float 6s ease-in-out infinite",fontFamily:C.mono }}>
      ...
      <div style={{ padding:18 }}>
        <div style={{ textAlign:"center",padding:"18px 0 16px",borderBottom:`1px solid ${C.border}`,marginBottom:16 }}>
          <div style={{ fontSize:9,color:C.muted,letterSpacing:2.5,marginBottom:6 }}>STRESS SCORE</div>
...
function StressRing({ score, color, size = 140 }) {
  const c = color || (score > 70 ? C.red : score > 45 ? C.amber : C.cyan);
  const label = score > 75 ? "CRITICAL STRESS" : score > 55 ? "HIGH STRESS" : score > 35 ? "MODERATE STRESS" : "LOW STRESS";
...
function StressBar({ label, score, color }) {
  const c = color || (score > 70 ? C.red : score > 45 ? C.amber : C.cyan);
  return (
...
function KpiCard({ label, value, delta, color, sub }) {
  const pos = String(delta).startsWith("+") || String(delta).includes("↑") || String(delta).toLowerCase().includes("low");
  return (
...
function AlertItem({ a, onEmail }) {
  ...
  return (
    <div style={{ background:`${c}08`,border:`1px solid ${c}30`,borderRadius:8,padding:"12px 14px",display:"flex",gap:10 }}>
      ...
      <button onClick={() => onEmail(a)} style={{ background: "transparent", border: "none", color: c, fontSize: 16, cursor: "pointer" }}>📧</button>
    </div>
  );
}
        ...
        <div style={{ display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center" }}>
          ...
          {filterOptions.map(f=>(
            <button key={f} onClick={()=>setFilter(f)}
              style={{ padding:"6px 12px",borderRadius:4,border:`1px solid ${filter===f?C.lime:C.border}`,background:filter===f?`${C.lime}18`:"transparent",color:filter===f?C.lime:C.muted,fontSize:10,fontFamily:C.mono,letterSpacing:1,cursor:"pointer",transition:"all 0.2s" }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:8,maxHeight:480,overflowY:"auto" }}>
          {filtered.map(c=>(
            <div key={c.id} onClick={()=>{setSelected(c);setResponse(c.response||"");}}
              style={{ background:selected?.id===c.id?`${C.lime}08`:C.surface,border:`1px solid ${selected?.id===c.id?C.lime+"40":C.border}`,borderRadius:8,padding:"14px 16px",cursor:"pointer",transition:"all 0.2s" }}>
        ...
        <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:24,position:"sticky",top:20,height:"fit-content",maxHeight:"85vh",overflowY:"auto" }}>
            ...
            <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
              {Object.entries(statusConfig).map(([k,{color,label}])=>(
                <button key={k} onClick={()=>handleUpdateStatus(selected.id, k)}
                  style={{ padding:"6px 12px",borderRadius:4,border:`1px solid ${selected.status===k?color:C.border}`,background:selected.status===k?`${color}25`:"transparent",color:selected.status===k?color:C.muted,fontSize:10,fontFamily:C.mono,cursor:"pointer",transition:"all 0.2s" }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <button onClick={()=>setResponse(AIResponses[selected.sentiment]||AIResponses.neutral)}
              style={{ width:"100%",padding:"10px 14px",background:`${C.cyan}12`,border:`1px solid ${C.cyan}35`,borderRadius:6,color:C.cyan,fontSize:11,fontFamily:C.mono,cursor:"pointer",textAlign:"left" }}>
          ...
          <div>
            ...
            <button onClick={handleSendResponse}
              style={{ width:"100%",marginTop:8,padding:"12px",background:response.trim()?C.lime:"rgba(200,255,0,0.1)",border:"none",borderRadius:6,color:response.trim()?C.bg:C.muted,fontSize:12,fontWeight:800,fontFamily:C.mono,cursor:"pointer",transition:"all 0.3s" }}>
...
function BusinessStressTab({ stress, sales, inv, sup, cash, complaints, role }) {
  const radarData = [
    { subject:"Sales", A: stress.breakdown.sales },
    { subject:"Cash Flow", A: stress.breakdown.cashflow },
    { subject:"Inventory", A: stress.breakdown.inventory },
    { subject:"Support", A: stress.breakdown.support },
    { subject:"Complaints", A: stress.breakdown.complaints },
  ];
  const catComplaintData = Object.entries(
    complaints.reduce((acc,c)=>{ acc[c.category]=(acc[c.category]||0)+1; return acc; },{})
  ).map(([name,value])=>({name,value}));
  const complaintTrend = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(d=>({
    d, open:Math.floor(Math.random() * (15 - 3) + 3), resolved:Math.floor(Math.random() * (18 - 5) + 5)
  }));
  const getStressAdvice = () => {
    const advice = [];
    if (stress.breakdown.sales > 50) advice.push({ icon:"📉", color:C.red, text:"Sales stress high. Review marketing spend and customer acquisition costs." });
    if (stress.breakdown.cashflow > 50) advice.push({ icon:"🏦", color:C.red, text:"Cash flow under pressure. Negotiate terms with suppliers or seek short-term financing." });
    if (stress.breakdown.inventory > 60) advice.push({ icon:"📦", color:C.amber, text:"Inventory stress elevated. Audit demand forecasting to prevent overstock." });
    if (stress.breakdown.support > 65) advice.push({ icon:"🎧", color:C.amber, text:"Support tickets spiking. Train staff or integrate AI triage." });
    if (stress.breakdown.complaints > 60) advice.push({ icon:"😡", color:C.red, text:"Complaint volume high. Analyze root causes to improve retention." });
    if (advice.length === 0) advice.push({ icon:"✅", color:C.lime, text:"Low stress across verticals. Focus on scaling opportunities." });
    return advice;
  };
  // Role-based view: Owner high-level, Ops detailed
  const ownerView = role === "owner";
  return (
    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:24 }}>
      <div>
        <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20,letterSpacing:0.5 }}>STRESS DIMENSION RADAR</h3>
        <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20,marginBottom:20 }}>
          <ResponsiveContainer width="100%" height={240}>
            <RadarChart data={radarData}>
              ...
              <Radar name="Stress" dataKey="A" stroke={stress.color} fill={stress.color} fillOpacity={0.15} strokeWidth={2} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
        {ownerView ? (
          <div>
            <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:14,letterSpacing:0.5 }}>EXECUTIVE SUMMARY</h3>
            <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20 }}>
              <p style={{ fontSize:12,color:C.text,lineHeight:1.6 }}>Overall stress: {stress.label}. Key risks: {Object.entries(stress.breakdown).filter(([k,v]) => v > 50).map(([k]) => k).join(", ") || "None"}.</p>
            </div>
          </div>
        ) : (
          <div>
            ...
            <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20 }}>
              {Object.entries(stress.breakdown).map(([k,v])=>(
                <StressBar key={k} label={k.toUpperCase().replace("_"," ")} score={v} />
              ))}
            </div>
          </div>
        )}
      ...
      <div>
        <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20,letterSpacing:0.5 }}>OVERALL BUSINESS STRESS</h3>
        <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:24,marginBottom:20,textAlign:"center" }}>
          <div style={{ display:"flex",justifyContent:"center",marginBottom:16 }}>
            <StressRing score={stress.overall} color={stress.color} size={160} />
          </div>
          <div style={{ fontFamily:C.display,fontSize:28,color:stress.color,letterSpacing:2 }}>{stress.label}</div>
          <div style={{ fontSize:12,color:C.muted,fontFamily:C.mono,marginTop:6 }}>COMPOSITE STRESS INDEX</div>
        </div>
...
function RevenueTab({ sales, role }) {
  const weeklyData = Array.from({length:7},(_,i)=>({
    day:["Mon","Tue","Wed","Thu","Fri","Sat","Sun"][i],
    rev:Math.floor(Math.random() * (180000 - 40000) + 40000), orders:Math.floor(Math.random() * (400 - 80) + 80), conv:parseFloat((Math.random() * (8.4 - 2.1) + 2.1).toFixed(1))
  }));
  // Role-based: Owner summary, Ops detailed charts
  const ownerView = role === "owner";
  return (
    <div>
      <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20 }}>SALES VERTICAL: 24H REVENUE STREAM</h3>
      {ownerView ? (
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{ fontSize: 48, color: C.lime, fontWeight: 800 }}>$${sales.reduce((s, d) => s + d.rev, 0).toLocaleString()}</div>
          <p style={{ fontSize: 12, color: C.muted, marginTop: 8 }}>Total Daily Revenue</p>
        </div>
      ) : (
        <>
...
function CashFlowTab({ cash, role }) {
  // Role-based
  const ownerView = role === "owner";
  return (
    <div>
      <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20 }}>CASH FLOW VERTICAL: 30-DAY POSITION</h3>
      {ownerView ? (
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{ fontSize: 48, color: C.purple, fontWeight: 800 }}>${Math.round(cash[cash.length - 1]?.bal / 1000 || 0)}k</div>
          <p style={{ fontSize: 12, color: C.muted, marginTop: 8 }}>Current Balance</p>
        </div>
      ) : (
        <>
          <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20,marginBottom:24 }}>
...
function InventoryTab({ inv, role }) {
  // Role-based
  const ownerView = role === "owner";
  return (
    <div>
      <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20 }}>INVENTORY VERTICAL: OVERVIEW</h3>
      {ownerView ? (
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{ fontSize: 48, color: inv.filter(i => i.stock < i.thr).length > 0 ? C.red : C.lime, fontWeight: 800 }}>{inv.filter(i => i.stock < i.thr).length}</div>
          <p style={{ fontSize: 12, color: C.muted, marginTop: 8 }}>Items Below Threshold</p>
        </div>
      ) : (
        <>
...
function SupportTab({ sup, role }) {
  // Role-based
  const ownerView = role === "owner";
  return (
    <div>
      <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20 }}>SUPPORT TICKETS VERTICAL: WEEKLY METRICS</h3>
      {ownerView ? (
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{ fontSize: 48, color: sup[sup.length - 1]?.escalated > 5 ? C.red : C.lime, fontWeight: 800 }}>{sup.reduce((s, d) => s + d.escalated, 0)}</div>
          <p style={{ fontSize: 12, color: C.muted, marginTop: 8 }}>Total Escalated Tickets</p>
        </div>
      ) : (
        <>
          <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:20,marginBottom:24 }}>
    ...
    <div style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",zIndex:10 }}>
      ...
      <div style={{ position:"relative",width:440,background:C.card,border:`1px solid ${C.lime}30`,borderRadius:12,padding:48,boxShadow:"0 20px 80px rgba(0,0,0,0.5)" }}>
        ...
        <button onClick={()=>name&&onComplete({name,industry})}
          style={{ width:"100%",padding:"18px",background:name?C.lime:"rgba(200,255,0,0.2)",border:"none",borderRadius:6,color:name?C.bg:C.muted,fontSize:14,fontWeight:800,fontFamily:C.mono,cursor:"pointer",transition:"all 0.3s",letterSpacing:1 }}>
          CONFIRM & LAUNCH →
        </button>
      </div>
    </div>
...
function Dashboard({ company, onBack }) {
  ...
  const [sup, setSup] = useState(genSupport());
  const [cash, setCash] = useState(genCash());
  const [complaints, setComplaints] = useState(genComplaints());
  const [role, setRole] = useState("owner");
  const [war, setWar] = useState(false);
  const [tab, setTab] = useState("overview");
  const [inputModal, setInputModal] = useState({ open: false, vertical: null });
  const stress = useMemo(() => calcBusinessStressScore(sales, inv, sup, cash, complaints), [sales, inv, sup, cash, complaints]);
  const alerts = useMemo(() => genAlerts(sales, inv, sup, cash, complaints), [sales, inv, sup, cash, complaints]);
  const handleDataLoad = (vertical, data) => {
    switch (vertical) {
      case "sales": setSales(data.length ? data : genSales()); break;
      case "inventory": setInv(data.length ? data : genInventory()); break;
      case "cash": setCash(data.length ? data : genCash()); break;
      case "support": setSup(data.length ? data : genSupport()); break;
      case "complaints": setComplaints(data.length ? data : genComplaints()); break;
    }
  };
  const handleEmailAlert = (alert) => {
    const subject = encodeURIComponent(`${alert.type.toUpperCase()} ALERT: ${alert.title}`);
    const body = encodeURIComponent(`${alert.msg}\n\nSent from OpsPulse Dashboard`);
    window.location.href = `mailto:ops@company.com?subject=${subject}&body=${body}`;
  };
  const totalRev = sales.reduce((s,d)=>s+d.rev,0);
  const lastCash = cash[cash.length-1]||{};
  const lastSup = sup[sup.length-1]||{};
  const openComplaints = complaints.filter(c=>c.status==="open"||c.status==="escalated").length;
  const ALL_TABS = ["Overview","Stress","Sales","Inventory","Cash Flow","Support","Complaints","Alerts"];
  return (
    <>
      <DataInputModal 
        isOpen={inputModal.open} 
        onClose={() => setInputModal({ open: false, vertical: null })} 
        onDataLoad={handleDataLoad} 
        vertical={inputModal.vertical} 
      />
      <div style={{ minHeight:"100vh",background:C.bg }}>
        ...
        <nav style={{ padding:"16px 32px",borderBottom:`1px solid ${C.border}`,background:`${C.bg}F0`,backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:war?27:0,zIndex:200,gap:12,flexWrap:"wrap" }}>
          <div style={{ display:"flex",alignItems:"center",gap:14 }}>
            <button onClick={onBack} style={{ padding:"6px 12px",fontSize:10,background:"transparent",border:`1px solid ${C.border}`,borderRadius:4,color:C.muted,fontFamily:C.mono,cursor:"pointer" }}>← EXIT</button>
            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:6,padding:"4px 10px",borderRadius:4,background:`${stress.color}15`,border:`1px solid ${stress.color}40` }}>
              <span style={{ fontSize:9,color:stress.color,fontFamily:C.mono,letterSpacing:1 }}>STRESS:</span>
              <span style={{ fontSize:11,fontWeight:700,color:stress.color,fontFamily:C.mono }}>{stress.overall} · {stress.label}</span>
            </div>
          </div>
          <div style={{ display:"flex",gap:10,alignItems:"center" }}>
            <div style={{ display:"flex",background:C.surface,borderRadius:6,padding:2,border:`1px solid ${C.border}` }}>
              {["owner","ops"].map(r=>(
                <button key={r} onClick={()=>{setRole(r);setTab("overview")}} style={{ padding:"6px 14px",borderRadius:4,border:"none",background:role===r?C.lime:"transparent",color:role===r?C.bg:C.muted,fontSize:11,fontWeight:700,fontFamily:C.body,cursor:"pointer" }}>
                  {r.toUpperCase()}
                </button>
              ))}
            </div>
            <button onClick={()=>setWar(!war)} style={{ background:war?C.red:`${C.red}15`,border:`1px solid ${C.red}`,color:war?"#fff":C.red,padding:"7px 14px",borderRadius:6,fontSize:10,fontWeight:800,fontFamily:C.mono,cursor:"pointer" }}>
              {war?"WAR MODE OFF":"LAUNCH WAR ROOM"}
            </button>
          </div>
        </nav>
        <div style={{ maxWidth:1280,margin:"0 auto",padding:"28px 32px" }}>
          {/* KPI row - Role-based KPIs */}
          <div style={{ display:"grid",gridTemplateColumns:"160px 1fr",gap:14,marginBottom:22 }}>
            <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:4 }}>
              ...
              <StressRing score={stress.overall} color={stress.color} size={130} />
            </div>
            <div style={{ display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12 }}>
              <KpiCard label="Daily Sales" value={`$${totalRev.toLocaleString()}`} delta="+12.4%" color={C.lime} />
              <KpiCard label="Low Inventory" value={inv.filter(i=>i.stock<i.thr).length} delta="Items Below Threshold" color={inv.filter(i=>i.stock<i.thr).length>0?C.red:C.cyan} />
          ...
          {/* Data Load Buttons - Focus on verticals */}
          <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
            {["sales", "inventory", "cash", "support", "complaints"].map(v => (
              <button key={v} onClick={() => setInputModal({ open: true, vertical: v })} style={{ padding: "8px 16px", background: C.surface, border: `1px solid ${C.border}`, color: C.muted, borderRadius: 4, cursor: "pointer", fontSize: 11, fontFamily: C.mono }}>
                Load {v.toUpperCase()} Data
              </button>
            ))}
          </div>
          {/* Tab nav */}
            ...
            {ALL_TABS.map(t=>{
              ...
              return (
                <button key={t} onClick={()=>setTab(key)}
                  style={{ padding:"8px 16px",borderRadius:6,border:`1px solid ${active?C.lime:C.border}`,background:active?`${C.lime}18`:"transparent",color:active?C.lime:C.muted,fontSize:11,fontWeight:active?700:400,fontFamily:C.mono,cursor:"pointer",transition:"all 0.2s",position:"relative",letterSpacing:0.5 }}>
                  {t}
          ...
          <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:24,minHeight:400 }}>
            {tab==="overview" && (
              <div style={{ display:"grid",gridTemplateColumns:"2fr 1fr",gap:24 }}>
                <div>
                  <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20 }}>SALES STREAM OVERVIEW</h3>
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
                  <h3 style={{ fontFamily:C.display,fontSize:22 }}>ACTIVE ALERTS (3-TIER)</h3>
                  <div style={{ display:"flex",flexDirection:"column",gap:8,maxHeight:300,overflowY:"auto" }}>
                    {alerts.length>0 ? alerts.map((a,i)=><AlertItem key={i} a={a} onEmail={handleEmailAlert} />) : (
                      <div style={{ textAlign:"center",padding:"40px 0",color:C.muted,fontFamily:C.mono,fontSize:12 }}>✓ ALL SYSTEMS NOMINAL</div>
                    )}
                  </div>
                  <div style={{ background:C.surface,border:`1px solid ${stress.color}30`,borderRadius:8,padding:14 }}>
                    <div style={{ fontSize:9,color:C.muted,fontFamily:C.mono,letterSpacing:2,marginBottom:8 }}>QUICK STRESS SNAPSHOT</div>
                    {Object.entries(stress.breakdown).map(([k,v])=>(
                      <StressBar key={k} label={k.toUpperCase()} score={v} />
                    ))}
                  </div>
                </div>
              </div>
            )}
            {tab==="stress" && <BusinessStressTab stress={stress} sales={sales} inv={inv} sup={sup} cash={cash} complaints={complaints} role={role} />}
            {tab==="sales" && <RevenueTab sales={sales} role={role} />}
            {tab==="cashflow" && <CashFlowTab cash={cash} role={role} />}
            {tab==="inventory" && <InventoryTab inv={inv} role={role} />}
            {tab==="support" && <SupportTab sup={sup} role={role} />}
            {tab==="complaints" && <ComplaintsTab complaints={complaints} setComplaints={setComplaints} />}
            {tab==="alerts" && (
              <div>
                <h3 style={{ fontFamily:C.display,fontSize:22,marginBottom:20 }}>ALL ALERTS (CRISIS / OPPORTUNITY / ANOMALY)</h3>
                <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
                  {alerts.length>0 ? alerts.map((a,i)=><AlertItem key={i} a={a} onEmail={handleEmailAlert} />) : (
...
function LandingPage({ onLaunch }) {
  ...
  const tickerItems = ["💰 SALES LIVE · UPDATING","📦 INVENTORY · OPTIMIZED","🏦 CASH FLOW · $124,000","🎧 SUPPORT TICKETS · NO ESCALATIONS","🚀 OPPORTUNITY SIGNAL","❓ ANOMALY FLAGGED","😡 COMPLAINTS · 3 OPEN","🩺 STRESS SCORE · 28"];
  return (
    <div style={{ background:C.bg,minHeight:"100vh" }}>
      ...
      <nav style={{ position:"fixed",top:0,left:0,right:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 48px",background:scrolled?"rgba(8,10,6,0.9)":"transparent",backdropFilter:scrolled?"blur(20px)":"none",borderBottom:scrolled?`1px solid ${C.border}`:"1px solid transparent",transition:"all 0.4s" }}>
        ...
        <button onClick={onLaunch} style={{ padding:"12px 28px",background:C.lime,border:"none",borderRadius:6,color:C.bg,fontSize:13,fontWeight:800,fontFamily:C.mono,cursor:"pointer" }}>Launch Console →</button>
      </nav>
      <section style={{ minHeight:"100vh",display:"flex",alignItems:"center",padding:"130px 80px 80px",position:"relative",zIndex:2,gap:60,flexWrap:"wrap" }}>
        <div style={{ flex:1,maxWidth:640 }}>
          ...
          <p style={{ fontSize:18,color:C.muted,lineHeight:1.85,maxWidth:500,marginTop:36,marginBottom:48,opacity:ready?1:0,transform:ready?"translateY(0)":"translateY(32px)",transition:"1s 0.1s cubic-bezier(0.16,1,0.3,1)" }}>
            Focus on core verticals: Sales, Inventory, Cash Flow, Support Tickets. Stress score from weighted analysis. 3-Tier alerts via Gmail. Dual views for Owner & Ops Manager.
          </p>
          <div style={{ display:"flex",gap:16,flexWrap:"wrap",opacity:ready?1:0,transform:ready?"translateY(0)":"translateY(24px)",transition:"1s 0.2s" }}>
            <button onClick={onLaunch} style={{ padding:"18px 48px",fontSize:16,background:C.lime,border:"none",borderRadius:6,color:C.bg,fontWeight:800,fontFamily:C.mono,cursor:"pointer" }}>🚀 Enter Dashboard</button>
            <button style={{ padding:"18px 32px",fontSize:14,background:"transparent",border:`1px solid ${C.border}`,borderRadius:6,color:C.muted,fontFamily:C.mono,cursor:"pointer" }}>Watch 2-min Demo</button>
          </div>
        </div>
